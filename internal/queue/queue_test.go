...
var _ queue.Queue[any] = &traceQueue{}

func TestBasic(t *testing.T) {
	tests := []struct {
		queue         *queue.Type
		queueShutDown func(queue.TypedInterface[any])
	}{
		{
			queue:         queue.NewTyped[any](),
			queueShutDown: queue.TypedInterface[any].ShutDown,
		},
		{
			queue:         queue.NewTyped[any](),
			queueShutDown: queue.TypedInterface[any].ShutDownWithDrain,
		},
	}
...
func TestCollapse(t *testing.T) {
	tq := &traceQueue{Queue: queue.DefaultQueue[any]()}
	q := queue.NewTypedWithConfig(queue.TypedQueueConfig[any]{
		Name:  "",
		Queue: tq,
	})
	// Add a new one twice
...
func mustGarbageCollect(t *testing.T, i interface{}) {
	...
	t.Cleanup(func() {
		...
		if atomic.LoadInt32(&collected) != 1 {
			t.Fatal("object was not garbage collected")
		}
	})
}
